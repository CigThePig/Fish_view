...
test("a mature ordinary plant can propagate and an immature one cannot", () => {
  ...
  const base = createAquariumState({ seed: 61 });
  const juvenile = {
    ...base,
  ...
  let spread = 0;
  {
    ...
    for (const seed of SEEDS) {
      ...
      for (const plant of state.plants) {
        ...
        spread = Math.max(spread, nearest);
        assert.ok(!plantSpecies(plant).rare, "a rare species propagated normally");
      }
    }
    assert.ok(checked > 0, `landscape produced no propagated plants to inspect`);
  }
  assert.ok(spread > 0);
});
...
test("propagation respects root spacing, local density, and the open-water gaps", () => {
  {
    ...
    for (const seed of SEEDS) {
      ...
      const state = grown(seed, 730);
      assert.ok(state.plants.length <= cap, `landscape/${seed} exceeded ${cap} plants`);
      assert.ok(state.plants.length >= plantCountFor());
      ...
      for (const plant of state.plants) {
        ...
        for (const other of state.plants) {
          ...
          assert.ok(Math.abs(other.x - plant.x) >= spacing - 1e-9,
            `landscape/${seed} rooted two plants ${Math.abs(other.x - plant.x)} apart`);
        }
      ...
      assert.ok(largestGap > state.cols * 0.055,
        `landscape/${seed} carpeted the substrate (largest gap ${largestGap.toFixed(2)})`);
    }
  }
});
...
test("a failed propagation opportunity is harmless rather than a retry storm", () => {
  ...
  assert.ok(filled.plants.length <= plantCapFor());
...
test("propagation unfolds over months instead of filling the tank in a fortnight", () => {
  {
    ...
    const cap = plantCapFor();
    const initial = plantCountFor();
    const counts = SEEDS.map((seed) => [14, 90, 180, 365].map((day) => grown(seed, day).plants.length));
    for (const [fortnight, quarter, half, year] of counts) {
      assert.ok(fortnight <= initial + 2, `landscape gained ${fortnight - initial} plants in two weeks`);
    ...
    assert.ok(averageHalf > initial + 1, `landscape barely changed in six months`);
    assert.ok(averageHalf < cap, `landscape was already at its cap by six months`);
  }
});

test("propagated plants begin as seedlings and later generations can follow", () => {
  const state = grown(4242, 500);
...
test("a delayed rare plant emerges as a real seedling near existing vegetation", () => {
  {
    ...
    for (const seed of SEEDS) {
      ...
      const later = grown(seed, milestone.day + 200);
...
test("plant caps are the tested budget rather than an aspiration", () => {
  assert.equal(PLANT_CAP, 30);
  assert.equal(plantCapFor(), 30);
  assert.ok(plantCapFor() > plantCountFor());
  assert.ok(plantCapFor() > plantCountFor());
});
