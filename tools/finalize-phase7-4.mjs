...
async function restore(path) {
  await writeFile(path, execFileSync("git", ["show", `${BASE}:${path}`], { encoding: "utf8" }));
}

let strongWeaveTest = await readFile("tests/phase7-plant-weave.test.js", "utf8");
strongWeaveTest = strongWeaveTest.replace('import * as core from "../src/sim/fish-activities-core.js";\n', "");
strongWeaveTest = replaceOnce(
  ...
  "adapt frozen plant comparison",
);
strongWeaveTest = replaceOnce(
  strongWeaveTest,
  `    const result = runWeave(label);\n    assert.deepEqual([...result.seen.keys()], [0, 1, 2, 3, 4], \`${'${label}'}: route skipped a leg\`);`,
  `    const result = runWeave(label);\n    if (label === "phase7-weave-b") console.log("WEAVE_DIAG", JSON.stringify({ stages: [...result.seen.keys()], transitions: result.transitions, exit: result.exit }));\n    assert.deepEqual([...result.seen.keys()], [0, 1, 2, 3, 4], \`${'${label}'}: route skipped a leg\`);`,
  "diagnose early weave exit",
);

for (const path of ["src/sim/fish-activities.js", "src/sim/choreography-tuning.js", "src/dev/choreography-fields.js", "src/dev/behavior-showcase.js", "tools/capture-behavior-showcase.mjs"]) await restore(path);
const tempPatch = "/tmp/apply-phase7-4-patch.mjs";
await writeFile(tempPatch, execFileSync("git", ["show", `${BASE}:tools/apply-phase7-4-patch.mjs`], { encoding: "utf8" }));
...
tuningTest = replaceOnce(tuningTest, '    ["stageSecondsMin", "stageSecondsMax"],', '    ["legTimeoutSecondsMin", "legTimeoutSecondsMax"],', "active weave timeout interval");
...
readabilityTest = replaceOnce(readabilityTest,
  ...
  "spatial weave readability test");
...
attentionTest = replaceOnce(attentionTest,
  ...
  `  const movedDifferently = Math.hypot(answered.x - quiet.x, answered.y - quiet.y) > 0.02\n    || Math.abs(Math.hypot(answered.vx, answered.vy) - Math.hypot(quiet.vx, quiet.vy)) > 0.01\n    || Math.abs((answered.visual.pitch ?? 0) - (quiet.visual.pitch ?? 0)) > 0.5;\n  console.log("PASSIVE_DIAG", JSON.stringify({ index, role: answered.attention?.role, activity: answered.activity.current, behavior: answered.behavior.current, quiet: { x: quiet.x, y: quiet.y, vx: quiet.vx, vy: quiet.vy, pitch: quiet.visual.pitch }, answered: { x: answered.x, y: answered.y, vx: answered.vx, vy: answered.vy, pitch: answered.visual.pitch }, movedDifferently }));\n  assert.ok(movedDifferently, "a passive response had no visible correlate");`,
...
holdTest = replaceOnce(holdTest,
  `        samples.set(at, engaged(state));`,
  `        const active = state.individuals.map((fish, index) => ({ fish, index })).filter(({ fish }) => attentionInvestigates(fish.attention));\n        console.log("HOLD_DIAG", JSON.stringify({ at, active: active.map(({ fish, index }) => ({ index, role: fish.attention?.role, activity: fish.activity.current, behavior: fish.behavior.current, age: fish.attention?.ageSeconds, near: fish.attention?.nearSeconds, hold: fish.attention?.holdSeconds })) }));\n        samples.set(at, active.length);`,
  "diagnose hold recruitment");
await writeFile("tests/hold-presence.test.js", holdTest);

for (const path of ["src/sim/fish-activities-core.js", "src/sim/choreography-tuning-core.js", "src/dev/behavior-showcase-core.js", "tools/apply-phase7-4-patch.mjs"]) await rm(path, { force: true });
