...
const results=[];
for(const days of [0,730]) {
  let damage=0, maxDamage=0, full=0, glyphs=0, objects=0, milliseconds=0, frames=0;
  for(const seed of options.seeds) {
    let state=advanceAquariumHistory(createAquariumState({seed}),days);
  ...
  results.push({days,frames,averageDamagePercent:damage/frames,maxDamagePercent:maxDamage,
