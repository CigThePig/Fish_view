...
This report is the acceptance evidence for Phase 6A–6E, including the product-first corrective review and the automated Codex review passes that followed the initial implementation. The repeatable measurement gate is `npm run measure:relationship`. The accelerated renderer capture is `npm run capture:relationship`.
...
- A restored session must run for at least one complete opportunity period before a voluntary glass visit can begin.
...
At high familiarity, each controlled personality produced a deterministic invitation within the one-hour scheduler scan after the post-session full-interval guard:
...
The accepted Phase 6 visual evidence generated by the verification workflow is committed with this report:

![Phase 6 relationship contact sheet](../assets/stage-2/phase-6/phase6-relationship-contact-sheet.png)

- [Phase 6 relationship capture manifest](../assets/stage-2/phase-6/phase6-relationship-capture-manifest.json)

Local developer captures may still be written to `.behavior-captures/`, and CI also retains its reduced-scale capture as a short-lived workflow artifact. The committed files above are the durable accepted evidence.
...
The automated review passes found additional seams after the first corrective review. Those are covered directly rather than suppressed:

- familiarity and saturation shaping reapply the Phase 2 responder budgets, preventing a familiar stocked tank from recreating a synchronized summons;
- a one-fish starting aquarium still guarantees a response to a fresh distant tap while preserving the original recovery thread;
- delayed responders receive moving-follow familiarity only after their response delay has actually elapsed;
- session/reload clocks stay closed for a complete opportunity period before voluntary visits are eligible, including the seed-2662 near-boundary case;
- active voluntary visits clamp touch-updated anchors before arrival testing, so edge and boundary touches remain reachable; and
- the accepted 3 × 4 renderer evidence and manifest are committed under `docs/assets/stage-2/phase-6/` and linked above.

The authoritative Stage 2 board marks Phase 6 Done and links this report, and this report closes with an explicit PASS verdict.

## Persistence and boundedness

The only new durable relationship datum is `history.glassFamiliarity`. Saturation, engagement clocks, recent viewer coordinates, and voluntary glass visits all live in the transient `viewerRelationship` record and are deliberately excluded from serialization. Existing persistence tests exercise old saves, malformed familiarity, round trips, offline advancement, the full post-restore invitation interval, and the fixed relationship footprint.
...
The strengthened product-first and Codex-reviewed implementation passed the complete gate after the latest runtime fixes:

- `npm test`: 484/484 passed;
...
- relationship measurement: controlled archetypes, production growth, spam resistance, natural autonomous visits, persistence boundedness, and personality separation all passed;
- relationship capture: the 3 personality × 4 familiarity contact sheet and manifest were generated successfully; and
- the exact accepted visual capture and manifest are now committed as durable evidence.

## Phase 6 acceptance

Phase 6 satisfies the Stage 2 gate:
...
- reloads cannot manufacture a near-immediate invitation by resetting the session clock;
...
- controlled familiarity comparisons, durable visual captures, a genuine production-growth path, and viewer-free natural initiation are all repeatable developer evidence.
